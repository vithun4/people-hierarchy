<template>
  <div class="flex flex-col items-center relative">
    <!-- Manager Node -->
    <div v-if="person"
      :class="['relative flex flex-col items-center rounded-lg shadow-lg hover:shadow-xl p-6 text-center w-64 border hover:border-gray-400 transition duration-200 ease-in-out', departmentColor]">
      <!-- Avatar or Initials -->
      <div
      ...
      <button v-if="countDescendants() > 0" @click="toggleExpand" class="text-sm focus:outline-none">
        <div
          :class="[isExpanded ? 'bg-gray-50' : 'bg-gray-200', 'text-gray-700 rounded-full px-3 py-1 text-xs mt-3 border border-gray-400 hover:bg-gray-100 transition-all duration-300 ease-in-out rounded-md']">
    ...
    <div v-if="isExpanded && subordinates.length > 0"
      class="flex flex-col items-center mt-4 p-4 border border-black border-t-4 border-0 transition-transform duration-300 ease-in-out"
      ...
      <div class="flex flex-row items-start space-x-4">
        <PersonNode v-for="subordinate in subordinates" :key="subordinate.EmployeeId" :person="subordinate"
          :departmentColors="departmentColors" :loadSubordinatesCallback="loadSubordinatesCallback" :level="level + 1"
